...
def log_probability(value, mean, variance):
    ...
    # Your code here
    prob = log(1.0 / pow(2 * kPI * variance, 0.5)) - (value - mean)**2 / (2 * variance)
    return prob
...
def republican_share(lines, states):
    ...
    share_vote_dict = {}
    for line in lines:
        if line["STATE"] in states and line["GENERAL %"] != "":
            if line["D"].isdigit(): 
                key = (line["STATE"], int(line["D"]))
            else:
                key = (line["STATE"], line["D"])
            value = float(line["GENERAL %"].replace(',', '.')[:-1])
            share_vote_dict[key] = value
    #return {("Alaska", 0): 50.97}
    return share_vote_dict
