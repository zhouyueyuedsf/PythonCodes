...
class BigramLanguageModel:
    ...
    def generate(self, context):
        ...
        # Your code here
        possible_bigram = {}
        for i in self._bigram_count:
            if context == i[0]:
                possible_bigram[i] = self._bigram_count[i]
        next_word = ""
        for k, v in possible_bigram.items():
            if v == max(possible_bigram.values()):
                next_word = k[1]
        return next_word
    ...
    def laplace(self, context, word):
        ...
        # Add your code here
        val = 0.0
        count = self._bigram_count
        singleCount = 0
        for c in count:
            if c[0] == context:
                singleCount += 1
        if (context, word) not in count:
            val = log(1.0 / (len(self._vocab) + singleCount))
        else:
            val = log(float(count[(context, word)] + 1) / (len(self._vocab) + singleCount))
        return val
    ...
    def add_train(self, sentence):
        ...
        for context, word in bigrams(list(self.tokenize_and_censor(sentence))):
            assert word in self._vocab, "%s not in vocab" % word
            self._bigram_count[(context, word)] += 1
