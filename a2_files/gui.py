...
WELCOME_MESSAGE = """
Welcome to Pokemon Finder {version}
Gotta Catch 'Em All!
""".format(version=VERSION).strip()

# default url for loading file from url
DEFAULT_LOAD_URL = "http://csse1001.uqcloud.net/a2/random-16-8.json"
...
class PokemonManager(object):
    ...
    @classmethod
    def init(cls, name_file="pokemon.txt"):
        """
        Initializes PokemonManager

        PokemonManager.init(PokemonManager, str) -> None
        """
        # load_file names
        cls._name_by_id = {}
        cls._id_by_name = {}
...
class PokemonApp(object):
    ...
    def __init__(self, master, welcome_message=WELCOME_MESSAGE):
        ...
        #self.load_game_file("game3.json")
    ...
    def load_game_url(self, url=None):
        ...
        except InvalidGameFile:
            messagebox.showerror("Invalid Game File", "The url {} could not be loaded.".format(url))
        except InvalidGameData:
            messagebox.showerror("Invalid Game Data", "The url {} does not contain valid game data.".format(url))
    ...
    def load_game_file(self, filename=None):
        ...
        try:
            ...
            messagebox.showerror("Invalid Game File", "The file {} could not be loaded.".format(filename))
        except InvalidGameData:
            messagebox.showerror("Invalid Game Data", "The file {} does not contain valid game data.".format(filename))
    ...
    def reset_pokemon(self, name=None):
        ...
        if self._playing:
            messagebox.showerror("Currently Playing", "Cannot reset Pokémon whilst playing a game!")
        else:
